...
(`FireDueReminders` + `ExpireStalePrompts`) and only returns once it fully
resolves (AC-01b) — the machine is not allowed to idle again mid-tick.
...
3. Point the external scheduler at `POST /wake` with the bearer token, every 3 minutes.
